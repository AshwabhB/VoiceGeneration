...
def extract_features_for_analysis(y, sr):
    ...
    #stress (using RMS energy as a proxy
    rms = librosa.feature.rms(y=y)[0]
    stress_mean = np.mean(rms)
    
    #speech rate(using onset detection)
    onset_env = librosa.onset.onset_strength(y=y, sr=sr)
    onset_frames = librosa.onset.onset_detect(onset_envelope=onset_env, sr=sr)
    speech_rate = len(onset_frames) / duration
...
def calculate_similarity(features1, features2):
    ...
    # Calculate similarity for linguistic features with high weights
    linguistic1 = features1['linguistic']
    linguistic2 = features2['linguistic']
    
    # Weights for linguistic features (normalized to sum to 0.9)
    linguistic_weights = {
        'pitch': 0.36,      # Most important for voice similarity
        'duration': 0.18,   # Important for speech timing
        'stress': 0.18,     # Important for emphasis
        'speech_rate': 0.18 # Important for rhythm
    }
    ...
    # Add voice quality similarity with weight 0.1
...
def calculate_overall_similarity(similarity_scores):
    ...
    weights = {
        'linguistic_pitch': 0.36,
        'linguistic_duration': 0.18,
        'linguistic_stress': 0.18,
        'linguistic_speech_rate': 0.18,
        'voice_quality': 0.1
    }
