...
def calculate_similarity(features1, features2):
    ...
    for feature_name in linguistic1.keys():
        ...
        similarity_scores[f'linguistic_{feature_name}'] = 1 - diff * linguistic_weights[feature_name]
    ...
    similarity_scores['voice_quality'] = 1 - voice_quality_diff * 0.1  # 10% weight
    
    # Add MFCC similarity
    mfcc1 = features1.get('mfccs', np.zeros((13, 1)))
    mfcc2 = features2.get('mfccs', np.zeros((13, 1)))
    mfcc_diff = np.mean(np.abs(mfcc1 - mfcc2))
    similarity_scores['mfccs'] = 1 - mfcc_diff * 0.25  # 25% weight
    
    # Add spectral features similarity
    spectral1 = features1.get('spectral', {})
    spectral2 = features2.get('spectral', {})
    
    # Spectral centroid
    centroid_diff = abs(spectral1.get('centroid', 0) - spectral2.get('centroid', 0))
    similarity_scores['spectral_centroid'] = 1 - centroid_diff * 0.1  # 10% weight
    
    # Spectral rolloff
    rolloff_diff = abs(spectral1.get('rolloff', 0) - spectral2.get('rolloff', 0))
    similarity_scores['spectral_rolloff'] = 1 - rolloff_diff * 0.1  # 10% weight
    
    # Spectral contrast
    contrast_diff = abs(spectral1.get('contrast', 0) - spectral2.get('contrast', 0))
    similarity_scores['spectral_contrast'] = 1 - contrast_diff * 0.1  # 10% weight
    
    # Tempo similarity
    tempo1 = features1.get('tempo', 0)
    tempo2 = features2.get('tempo', 0)
    tempo_diff = abs(tempo1 - tempo2) / (tempo1 + 1e-6)
    similarity_scores['tempo'] = 1 - tempo_diff * 0.05  # 5% weight
...
def calculate_overall_similarity(similarity_scores):
    ...
    weights = {
        ...
        'tempo': 0.05,            # Rhythm
        'voice_quality': 0.1,     # Voice quality
    ...
    return float(overall_score)  # Ensure we return a float, not a numpy array
